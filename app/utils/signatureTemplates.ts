...
export const TEMPLATES: TemplateInfo[] = [
  { id: 'modern', name: 'Moderno', description: 'Layout limpo com gradiente lateral', premium: false, preview: '🎨' },
  { id: 'classic', name: 'Clássico', description: 'Estilo corporativo tradicional', premium: false, preview: '📋' },
  { id: 'minimal', name: 'Minimalista', description: 'Foco no essencial, sem distrações', premium: true, preview: '✨' },
  { id: 'bold', name: 'Ousado', description: 'Cores vibrantes e tipografia forte', premium: true, preview: '🔥' },
  { id: 'elegant', name: 'Elegante', description: 'Sofisticado com detalhes refinados', premium: true, preview: '💎' },
  { id: 'creative', name: 'Criativo', description: 'Layout assimétrico e moderno', premium: true, preview: '🚀' },
]

function socialIcon(url: string, label: string, color: string): string {
  if (!url) return ''
  return `<a href="${url}" style="color:${color};text-decoration:none;margin-right:8px;font-size:13px;" target="_blank">${label}</a>`
}

function qrCell(whatsappUrl: string, qrDataUrl: string): string {
...
export function renderModern(data: SignatureData, qrDataUrl: string): string {
  const { name, title, email, phone, location, photoUrl, linkedinUrl, githubUrl, whatsappUrl, calendlyUrl, portfolioUrl, availability, availabilityHours, companyLogoUrl, primaryColor, secondaryColor } = data
...
export function renderMinimal(data: SignatureData, qrDataUrl: string): string {
  const { name, title, email, phone, primaryColor, secondaryColor, linkedinUrl, githubUrl, whatsappUrl } = data
...
export function renderSignature(data: SignatureData, qrDataUrl: string): string {
